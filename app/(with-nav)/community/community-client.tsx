...
import type { CropSettings, PhotoDraft } from "./community-photo-processing";
import {
...
type CommunityClientProps = {
  ...
  initialMemberProfiles?: MemberProfile[];
};
...
export default function CommunityClient({
  ...
  const [memberProfiles, setMemberProfiles] = useState<MemberProfile[]>(initialMemberProfiles);
  ...
  const hasLoadedInitialPostsRef = useRef(initialPosts.length > 0);

  const loadPhotoProcessing = useCallback(
    () => import("./community-photo-processing"),
    [],
  );
  ...
  useEffect(() => {
    if (hasLoadedInitialPostsRef.current) {
      hasLoadedInitialPostsRef.current = false;
      return;
    }
    void loadPosts();
  }, []);
  ...
  const onPhotoSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { createPhotoDraft } = await loadPhotoProcessing();
      const nextDrafts = await Promise.all(
        acceptedFiles.map((file) => createPhotoDraft(file, MAX_PREVIEW_PHOTO_DATA_URL_BYTES)),
      );
  ...
  const saveCropChanges = async () => {
    ...
    try {
      const { renderCroppedImageDataUrl } = await loadPhotoProcessing();
