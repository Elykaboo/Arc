...
const deleteByCollectionGroupField = async (
  ...
  while (true) {
    const snapshot = await db.collectionGroup(collectionName).where(fieldName, "==", value).limit(DELETE_BATCH_SIZE).get();
...
const deleteByCollectionGroupDocumentId = async (
  collectionName: string,
  documentId: string,
): Promise<number> => {
  const db = await getAdminDb();
...
export const deleteUserAccountAsAdmin = async (input: {
  ...
  commentsDeleted += await deleteByCollectionGroupField("communityComments", "uid", targetUid);
  likesDeleted += await deleteByCollectionGroupField("communityLikes", "uid", targetUid);

  followEdgesDeleted += await deleteByCollectionGroupDocumentId("following", targetUid);
  followEdgesDeleted += await deleteByCollectionGroupDocumentId("followers", targetUid);
  notificationsDeleted += await deleteByCollectionGroupField("notifications", "actorUid", targetUid);
